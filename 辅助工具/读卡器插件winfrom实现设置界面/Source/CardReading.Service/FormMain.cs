...
namespace CardReading.Service
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            //本地ip
            var bdip = ips.Aggregate(string.Empty, (current, ip) => current + ip + Environment.NewLine);
            ...
            _serviceUrl = string.Format("http://127.0.0.1:{0}/CardRead", ServicePort);
            //不是第一次运行的时候 开启测试按钮
            button1.Enabled = true;
            try
            {
                var hh = Common.CreateServiceHost(typeof(CardRead), _serviceUrl);
