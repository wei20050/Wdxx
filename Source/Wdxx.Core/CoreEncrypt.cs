...
namespace Wdxx.Core
{
    ...
    public static class CoreEncrypt
    {
        ...
        public static void DesEncryptFile(string mInFilePath, string mOutFilePath, string strEncrKey)
        {
            ...
            {
                throw new Exception("DES加密文件发生异常:" + ex);
            }
        ...
        /// 默认AesKey
        /// </summary>
        private const string DefaultAesKey = "@#CoreEncrypt88888888";
        ...
        public static bool AES_EncryptFile(string inputFile, string outputFile)
        {
            try
            {
                ...
                var enfr = AES_EncryptStrream(fren, DefaultAesKey);
        ...
        public static bool AES_DecryptFile(string inputFile, string outputFile)
        {
            try
            {
                ...
                var defr = AES_DecryptStream(fr, DefaultAesKey);
        ...
        public static bool AddMd5(string path)
        {
            ...
            try
            {
                ...
                {
                    // 对Buffer中的字节内容算MD5
                    var result = Md5Buffer(md5File, 0, md5File.Length);
                    // 将字符串转换成字节数组以便写人到文件中
                    var md5 = Encoding.ASCII.GetBytes(result);       
        ...
        public static bool CheckMd5(string path)
        {
            try
            {
                ...
                // 读入文件
                var md5File = new byte[getFile.Length];                                      
                getFile.Read(md5File, 0, (int)getFile.Length);
                getFile.Close();
                // 对文件除最后32位以外的字节计算MD5，这个32是因为标签位为32位。
                var result = Md5Buffer(md5File, 0, md5File.Length - 32);
                //读取文件最后32位，其中保存的就是MD5值
                var md5 = Encoding.ASCII.GetString(md5File, md5File.Length - 32, 32);   
                return result == md5;
            }
            catch
            {
